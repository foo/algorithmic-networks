...
In this chapter, we introduce a natural extension of an online caching problem, where
items have inter-de\-pen\-den\-cies.
In classic online caching problem, items of some universe are requested by
...
Then, we introduce the formal model for \emph{caching with tree dependencies}, for which we propose a competitive algorithm.
In Section~\ref{sec:bisimulation} we establish a connection between caching with tree dependencies and caching of forwarding rules.
...
software controller. In particular, our
...
However, if $v$ is not present in the cache, no other rule present in the cache matches the packet, as predecessors of $v$ do not match the packet.

Consider the situation, where that the router does not have sufficient information to proceed.
The common way to handle this is to redirect the packet to the controller (where 
...
We will show that such penalties can be easily simulated in our model, see Section~\ref{sec:bisimulation}.
