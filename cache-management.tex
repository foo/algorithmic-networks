...
Then, we introduce an abstract problem of \emph{tree caching} which, for which we propose a competitive algorithm.
...
Network (SDN) technology, which allows managing the router forwarding table using a
software controller.
...
To see that this is sufficient, consider a packet that, having access to the complete forwarding table, would be forwarded by the rule $v$.
...
For this indirection in packet forwarding, we assign a fixed cost~$1$.
...
cost of $\alpha$ for updating the router.
...
The ability to process negative requests will be crucial in using our algorithm to handle forced cache modifications (cf. Section~\ref{sec:rule-up}), and we elaborate on this in Section~\ref{sec:bisimulation}.
...
some elements, then $A_\textnormal{T}$ bypasses $\alpha$ corresponding requests
...
a $2 q$-competitive online algorithm~$A$ for the tree caching problem with forced updates.
