...
where we explain the setup for \emph{caching of forwarding rules} in a router.
Then, we introduce an abstract problem of \emph{caching with tree dependencies} which, with minor differences, matches the forwarding rules caching problem.
For caching with tree dependencies we propose a competitive algorithm.
...
In such a case, they are called control plane and forwarding plane, respectively.

Rules in the forwarding table form a tree.
That is, the forwarding rules are prefixes of
...
To handle packets that would be matched by a rule not present in the cache, we want to 
 redirect them to the controller, where 
...
For this indirection in packet forwarding, we charge the algorithm the cost~$1$.
...
At some chosen points in time, the caching algorithm run at the controller may decide
...
In Section~\ref{sec:bisimulation}, we will show that such penalties can be easily simulated in our model.
...
The ability to process negative request will be crucial in using our algorithm to handle rule updates, as described in Section~\ref{sec:rule-up}.
...
Namely, we take any input $I$ for the latter problem and create, in an online
