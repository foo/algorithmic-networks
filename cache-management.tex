...
Network (SDN) technology, which allows to manage the router forwarding table using a
software controller.
...
In such case, they are called control plane and forwarding plane, respectively.
...
(i.e., keeping a~tree node in cache while evicting its descendant) would result
...
This rule is the least specific rule for any packet, hence as long as any rule that matches the packet is present in cache, the LMP scheme never selects the artificial rule.
...
In this chapter we define a novel variant of online caching which finds
