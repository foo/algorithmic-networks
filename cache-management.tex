...
To model the problem of maintaining large forwarding tables, we introduce a natural extension of an online caching problem, where
...
We model this setup by introducing an abstract problem of \emph{tree caching} which, for which we propose a competitive algorithm.
...
One of the solutions is to store only a part of a~forwarding table in the router, which acts as a cache for the complete forwarding table.
...
We ignore this issue for now, and we come back to it in Section~\ref{sec:bisimulation}, 
...
In the standard caching problem, fetching is mandatory: the requested item must be present in the cache to serve the request,
...
In Lemma~\ref{lem:no_over-requested_changesets}, we state that no valid changeset contains too many requests to its rules.
...
We call this the \emph{extended tree caching problem}.
...
To this end, we repeat positive and negative requests from $I$, and upon encountering a forced cache modification of some node, we insert a sequence $\alpha$ negative requests for it, called a \emph{chunk}.
