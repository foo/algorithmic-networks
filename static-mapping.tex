...
In the $\CTE$ variant without $\MA$, each node has exactly one chunk assigned, and $\MaFactor = 1$.
...
We first present an algorithm to solve the~$\RS+\MA+\NI+\BW$ variant of $\CTE$ problem.
...
Furthermore, we reduce bandwidth capacities so that the volume of capacity corresponds to number of chunks that we transfer through the link.
Concretely, for~$\Tree^*$, we set the bandwidth of~$\Tree$ to integer multiples of~$\CostTrans$,
...
The~maximum flow with minimal costs is indicated by the dashed lines: each line
...
Using the double scaling algorithm for min-cost-max-flow, we get a runtime of~$\mathcal{O}(n_S^2\cdot m \cdot\log m \cdot \log U \cdot \log n)$
where~$U$ is the maximal link capacity. Note that each edge has a constant cost, and in networks with high capacity
...
the two variants of $\CTE$ problem, the $\RS+\MA+\NI$ and~$\MA+\NI+\BW$, that can also be solved with the flow approach
introduced above.
...
which in a tree can be computed in time~$\mathcal{O}(n_S)$; thus, the overall construction time
is
$\mathcal{O}(n_S \cdot \tau^2)$.
The state-of-the-art algorithm to compute matchings~\cite{matching-best} has a running time of $\mathcal{O}(\tau^{10/7}\cdot \log(\tau\cdot n_S))$; recall that~$\tau = \MaFactor\cdot n_V$.


\subsubsection{Faster~$\MA+\NI$ and~$\MA+\NI+\BW$}

We now show that we can solve~$\MA+\NI$ even faster, by exploiting
...
The matching-based algorithm outperforms flow-based algorithm in situations, where the substrate network is large in comparison to the size of virtual network, as the matching-based algorithm is linear in terms of $n_S$, and the flow-based algorithm is quadratic in terms of $n_S$.
...
with the
remaining parts of~$\Tree$, which inflicts costs~$\CostTrans$ per excess chunk on the uplink of root of~$\Tree'$.
 Similarly, if the processing capabilities exceed the
amount of
available chunks, excess chunks from other subtrees have to be transferred
to
...
Indeed, the~$\MA+\FP+\NI+\BW$ variant
