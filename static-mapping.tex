...
both sets are non-empty, we greedily match an arbitrary chunk in~$S_c$ with an arbitrary node in~$S_v$,
and remove them from the sets.\todo{!}

\parag{Analysis.} On a given vertex~$u$, emptying one of the sets, results in a \emph{local assignment}
in the
...
The local matching algorithm outperforms the previous presented matching-based algorithm.
...
\todo{Algorytm osiaga tyle ile w lemacie. Dlatego jest optymalny}
