...
\caption{Fastest algorithms for different respective variants. Variants depicted by solid black are NP-hard, and variants depicted by checked filling are trivially solvable. For the remainder of variants we provide the fastest algorithm.}
\label{fig:venn_full}
\end{figure}
...
\parag{Algorithm.} Due to Observation~\ref{obs:nofp},~$\RS+\MA+\NI$ variant degenerates to~$\RS+\MA$.
...
In this section, we present the way to solve~$\MA+\NI$ variant even faster by exploiting
...
\parag{Algorithm.} Our proposed algorithm for $\CTE$ variants with $\MA$
...
leverages the \emph{optimal substructure property} of~$\MA+\FP+\NI+\BW$ variant:
...
Our proof that the variant $\FP+\RS+\MA$ of the $\CTE$ problem is NP-hard is based on the following main ideas.
We encode a~$\TDM$ instance as an instance as follows:
...
\parag{Example.} Figure~\ref{fig:fprsma} shows an example of our construction: In an~instance~$I$
...
$\FP+\RS+\MA$ variant constructed as described above. We prove that~$I'$ has a solution of cost at most $\Thr = 4 \cdot k$ if ($\Rightarrow$) and only if
($\Leftarrow$)
...
($\Leftarrow$) Let us take a solution to~$\FP+\RS+\MA$ variant of cost at most $\Thr$. We
...
for~$\FP+\RS+\NI$ variant as follows:
\begin{itemize}
...
$\FP+\RS+\NI$ variant into a decision problem. A~detailed explanation of this value can
...
\begin{proof}
Let~$I$ be an instance of~$\TDM$ and let~$I'$ be an instance of
$\FP+\RS+\NI$ variant constructed as described above.
...
In Section~\ref{ap:tworep-ma} we provide the hardness result for $\RS(2)+\MA+\FP$ variant, and in Section~\ref{ap:tworep-ni} we provide the hardness result for $\RS(2)+\FP+\NI+\BW$ variant.

Both variants are reduced from the problem $\TDPM$ (see Section~\ref{sec:3dm_intro}).
The constructions are based upon the reduction of $\TDPM$ to $\FP+\RS+\MA$ variant of $\CTE$ (see Section~\ref{ssec:fprsma}) and the reduction of $\TDPM$ to $\FP+\RS+\NI$ variant (see Section~\ref{ssec:fprscc}).
...
\parag{Construction.} For an arbitrary instance~$\ITDPM$ of~$\TDM$ we construct a~$\RS(2)+\MA(4)+\FP$ variant instance~$\IVCEMB$ the way described in the remainder of this section.
Let $k = |X|=|Y|=|Z|$.
...
\begin{enumerate}
 ...
 ~$\numNodes = k + \sum_{e}(\deg(e)-1) = 3\cdot p - 2\cdot k$ nodes.
...
Given any $\TDPM$ instance $\ITDPM$, we produce corresponding instance of $\VCEMB$ variant, namely the $\RS(2)+\MA+\FP$ variant instance, in the way described above.
We refer to such $\RS(2)+\MA+\FP$ variant instance as the $\IVCEMB$.
...
Using the fact that $\sum_e(\deg(e)-1) = 3\cdot p - 3\cdot k = \Vms - k$, by pidgeon-hole principle we conclude that indeed spawning $\Vms-k+i$ nodes in the \CoverSubtree{} results in at least $3\cdot i$ long matches.
...
We now show that~$\RS(2)+\FP+\NI+\BW$ variant is even NP-hard without multiple
assignment.
The proof is similar in spirit to proof of hardness of $\RS(2)+\FP+\MA$ variant.
...
\begin{enumerate}
  ...
  a lower and an upper middle node. Note that this is different from $\RS(2)+\FP+\MA(4)$ variant NP-completeness proof, where {\UnqSubtree} was placed in the {\MatchSubtree}.
...
Similar in spirit to the NP-completeness proof of~$\RS(2)+\MA(4)+\FP$ variant,
