...
\begin{enumerate}
  ...
  \item The {\TripleGadget} consists of four vertices: three leaves and the root of the gadget, as defined in the reduction for the $\FP+\RS+\MA$ variant in Section~\ref{ssec:fprsma}.
...
The other two sets of chunks has one replica.
...
\begin{enumerate}
  ...
  For each $e$ and $t\in P_e$, we then have a chunk $c(e, t)$. It has two replicas:
  $c^{(1)}(e,t)$ and $c^{(2)}(e,t)$.
...
The multi-assignment factor is $m=4$, and the number of nodes to embed is~$\numNodes = k + \sum_{e}(\deg(e)-1) = 3\cdot p - 2\cdot k$.
 ...
 The threshold value corresponds to the cost of solution, where $k$ nodes are matched to $4$~chunks that are in distances: $0, 2, 2$ and $4$ to the node, and remaining $\Vms-k$ nodes are matched to $4$~chunks that are in distances: $0, 2, 2$ and $2$ to the node.
...
In the instance $\IVCEMB$, chunks can be matched to nodes at distance $0, 2, 4$ or $6$.
  ...
  \begin{proof}
    ...
    In total, the cost of the solution is $4\cdot (k+i) + 2\cdot (3\cdot n_V - k-i) = 6\cdot n_V + 2\cdot k + 2\cdot i = \Thr + 2\cdot i > \Thr$.
  \end{proof}
