...
+ \RS$ variant of the $\CTE$ problem. In specifying the chunk placement we ommit the replica subscripts. Each triple corresponds to a single triple gadget.}
\hfill
\label{fig:fprsma}
\end{figure}
...
\caption{An instance of $\FP+\MA+\RS(2)$ variant of $\CTE$ corresponding to the $\TDM$ instance from Figure~\ref{fig:fprsma}. Leaves containing dummy chunks are grey. The optimal solution maps chunks to nodes $v_1, \ldots v_7$ by the dashed lines. In specifying the chunk placement we ommit the replica subscripts.}
...
Given any $\TDPM$ instance $\ITDPM$, we produce the corresponding instance $\ICTE$ of the $\RS(2)+\MA+\FP$ variant, in the way described above.
\maciek{todo: wstep. Reference to Figure~\ref{fig:example-rs2}}
\begin{lemma}
...
\begin{proof}
  ...
  \begin{enumerate}
   ...
   ~$\deg(e) - 1$ nodes and assign them to arbitrary chunks in this
    gadget, which are not yet assigned in any {\TripleGadget}.
  \end{enumerate}
...
\todo{wstep ze teraz w druga strone}
...
\begin{proof}
  ...
  By Lemma~\ref{lem:no-unq-ma}, these $k$ long assignments are depleted by $k$ unique chunks, and remaining chunks are assigned by a short assignment.
