...
Let~$I$ be an instance of~$\TDM$ with $t$ triples and set cardinality $k$ (where $k = |X| = |Y| = |Z|$).
...
$\FP+\RS+\MA$ constructed as described above. We prove that~$I'$ has a solution of cost at most $\Thr = 4 \cdot k$ if ($\Rightarrow$) and only if
($\Leftarrow$)
...
\begin{enumerate}
  \item For each triple $\tau \in P$, we construct $3$ chunks, with two replicas each.
...
\begin{enumerate}
  ...
  {\MatchSubtree} consists of $t$ {\TripleGadgets}, one per each triple $\tau\in P$ and $k$
...
\begin{enumerate}
 ...
 We put replicas $ch_2(\tau, e)$ for each $\tau \in P_e$.
...
\begin{proof}
  ...
  By Lemma~\ref{lem:short-ma}, each match of $ch(e, \tau)$ for each $e\in X\cup Y\cup Z$ and each $\tau \in P$ is matched by a short match.
  Hence, each active node processes the 3 chunks that are placed in its \TripleGadget.
  In each {\ElGadget} for element~$e$, one chunk $ch(e, \tau)$ for some $\tau \in P$ is not matched.
