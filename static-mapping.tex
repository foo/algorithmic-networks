...
(2) the data needs to be processed (divided into data chunks), and
...
as the \emph{vertices}. We will assume that the inter-connecting network forms an arbitrary tree,
...
The data is stored in a distributed filesystem in the data-center; this spatial distribution is given and not subject to optimization.
The input data consists of~$\tau$ different \emph{chunks}~$\achunk_1, \ldots, \achunk_{\ChunkType}$,
where each chunk~$\achunk_i$ can have~$r_i\geq 1$ instances (or replicas)~$\achunk_{i}^{(1)},\ldots, \achunk_{i}^{(r_i)}$,
...
\parag{Replica Selection ($\RS$).} The first fundamental problem is replica selection:
...
\parag{Flexible Placement ($\FP$).} %The third fundamental degree of freedom, besides replica selection and assignment,
...
\parag{Node Interconnect ($\CC$).} We distinguish between scenarios
