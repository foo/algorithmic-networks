...
The threshold value is $\Thr := 6\cdot k + 18\cdot (k-1)\cdot k$, and we set the access cost~$\CostTrans$ to a chunk replica to $\Thr + 1$.
...
directly at different $3\cdot k$ chunks, resulting in 0 costs for the access network.
...
is chosen so that it is never possible to distribute nodes
...
(1) the inter-connect cost inside a gadget is~$2 \cdot {3 \choose 2}$,
  ...
  \cdot 3 \cdot 3 \cdot (k - 1) / 2$, where the factor~$4$ corresponds to the distance $4$, the factor~$3$
