In large-scale batch-processing systems such as MapReduce~\cite{mapreduce}, multiple physical machines cooperate to processes large quantities of data stored in the data center in a distributed manner.
For a certain computational task, one spawns an adequate number of worker to process the data, whose task is to select its data portion, and process it, and aggregate the result by cooperation with other workers.
...
While nodes of a virtual network can be placed at any feasible physical machine in the data center, the performance of the processing is heavily influenced by their placement.
Placing nodes closely to eachother reduces network latency and bandwidth reservations in the data center.
...
\todo{worker in the intro}
\todo{remove chunk type}
...
as well as to the chunks), and (3) the data needs to be processed (divided into data chunks).
...
as the \emph{vertices}. We assume that the inter-connecting network forms an arbitrary tree (\maciek{rooted?}),
...
\parag{The Virtual Network.} The virtual network consists of a set~$\VirtualNodes$ of~$n_V=|\VirtualNodes|$ virtual machines,
henceforth often simply called \emph{nodes}.
Each node~$v \in \VirtualNodes$ can be placed (or, synonymously, \emph{embedded}) on an arbitrary server; this placement is a subject
to optimization.

\parag{The Data.} The to be processed data constitutes the input to the batch-processing application.
The data is stored in a distributed filesystem spread across the servers; this spatial distribution is given and not subject to optimization.
...
It is sufficient to process one replica, and we sometimes refer to this
...
We denote the server~$s$ hosting node~$v$ by~$\pi(v)=s$.
...
minimal bandwidth guarantees; we refer to the first type of virtual network as the \emph{(chunk) access
...
its assigned (active) chunk replica~$\achunk$, and let~$\dist(v_1,v_2)$ denote the distance between the two nodes~$v_1$ and~$v_2$ \maciek{(we use the hop count and dist interchangeably)?}.
We define the \emph{footprint}~$\Cost(v)$ of a node~$v$ as follows:
$$
\Cost(v) = \underbrace{\sum_{\achunk\in \mu(v)} \CostTrans \cdot \dist(v,\achunk)}_{chunk access network} +  \underbrace{\frac{1}{2} \cdot \sum_{v' \in \VirtualNodes\setminus\{v\}} \CostCom \cdot \dist(v,v')}_{\text{node inter-connect}},
$$
where~$\mu(v)$ is the set of chunks assigned to~$v$. Our goal is to minimize the overall resource footprint $\Cost=\sum_{v\in V} \Cost(v)$.

\maciek{Tu info o constraintach: co najwyzej $cap(s)$ node w serwerze; respect all the links}

\subsection{Problem Decomposition}

In order to chart the landscape of the computational tractability and intractability of different
problem variants, we decompose our problem into its fundamental aspects, namely multiple chunk assignment ($\MA$),
replica selection ($\RS$), flexible node placement ($\FP$), node interconnect ($\CC$),
and bandwidth constraints ($\BW$), as described in the following. 
In this chapter, we consider all possible $32$ problem variants, where each of these five aspects
...
We investigate assignments that balances chunks among nodes, i.e. where the number of chunks is divisible by the number of nodes, and each node processes an identical integer number of chunks~$\MaFactor = \tau / n_V$, and we refer to $\MaFactor$ as the multi-assignment factor.
We refer to such scenario as $\MA$.
In the problem variant without $\MA$, each node has exactly one chunk assigned.

\parag{Replica Selection ($\RS$).}
Distributed filesystems often utilize data redundancy for corruption detection and correction.
A redundant data chunk has multiple \emph{replicas}, often stored on different physical machines.
...
In the problem variant without $\RS$, a node needs to access the singular location where the assigned chunk resides.

\parag{Flexible Placement ($\FP$).}
In most applications, nodes of virtual network are indifferent to their physical placement.
...
\parag{Node Interconnect ($\CC$).}
In multiple computational tasks, it is sufficient to process data chunks independently of eachother.
However, more often the result of processing the individual chunks is combined afterwards.
By $\CC$ we refer to the latter scenario, where the computation requires the nodes to cooperate.
We investigate the scenario, where we reserve a bandwidth of volume $\CostCom$ between each pair of nodes, i.e. the node interconnect is modelled as a complete graph, to account for the all-to-all communication patterns of batch processing applications such as MapReduce.
In the problem variant without $\CC$, we optimize only the cost of transporting chunks to nodes.
...
In the problem variant without $\BW$, each link has infinite capacity.
...
We first present an algorithm to solve the~$\RS+\MA+\CC+\BW$ variant.
...
capacity in~$\Tree^*$ to~$\lfloor\capacity(e)/\CostTrans\rfloor$.\todo{font?}
...
Exemplary problem instance is presented in Figure~\ref{fig:flow_construction}.
...
among all other fractional paths of the same length\todo{opisz dokladniej}; due to the optimality of the fractional solution
...
dominant term in the asymptotic runtime is the flow computation\todo{upewnij sie}.
...
\subsection{Matching Algorithms for $\RS+\MA+\CC$}\label{ssec:match}
...
at fixed locations (no $\FP$). The number of chunk types may be larger than the number
of nodes ($\MA$), and each node needs to be connected
to its chunks as well as to other nodes ($\CC$).

\parag{Algorithm.} Due to Observation~\ref{obs:nofp},~$\RS+\MA+\CC$ degenerates to~$\RS+\MA$.
...
single %$\ChunkType$
``super-vertex''.
For each node $v$ and chunk $c$, we connect $v$ and super-vertex $c$ with the link of cost $\min_i \dist(v,c^{(i)})$, i.e. the cost of reaching the closest replica.
On the resulting bipartite graph, we compute a \emph{minimum weight
perfect
matching}~\cite{gabow_scaling_algorithm}:
the resulting matching describes the optimal assignment of chunks to nodes.
Exemplary problem instance is presented in Figure~\ref{fig:matching}.
...
\caption{The~$\RS+\MA$ problem on the \emph{left} is converted into a
  matching problem on the \emph{right}.
...
which in a tree can be computed in time~$\O(n_S)$; thus, the overall construction time
is
...
\todo{Porownaj z flow, kiedy ktore jest lepsze}
...
\parag{Analysis.} On a given vertex~$u$, emptying one of the sets, results in a \emph{local assignment} (cf~Definition~\ref{def:loc})
in the
...
\begin{figure}[t]
\hfill
\includegraphics[width = 0.18\columnwidth]{figs/static-mapping/np_3dm_formular}
\hfill
\includegraphics[width = 0.35\columnwidth]{figs/static-mapping/np_3dm_construction}
...
\parag{Families of chunk types.} The second tool that we introduce allows us to express the redundancy of chunks without actually replicating chunks more than two-fold.
...
Namely, for each element of universe $e$, we construct as many chunk types as there are occurences of $e$ in triples of $\ITDPM$.
...
\parag{Contruction.}
For an arbitrary instance~$\ITDPM$ of~$\TDM$ we construct a~$\RS(2)+\FP+\CC+\BW$ instance~$\IVCEMB$ the way described in the remainder of this section.
Let $k = |X|=|Y|=|Z|$.
By $T$ we denote the set of all triples of $\ITDPM$, and let $t = |T|$.
For each $e\in X\cup Y\cup Z$, by $T_e$ we denote the set of all triples that contain element $e$.
Let $\deg(e) = |T_e|$, and note that $\sum_e \deg(e) = 3\cdot t$.
...
\begin{proof}
 ...
 ~$I'$ has solution of cost~$\leq \Thr$ if and only if~$I \in \TDPM$
  ...
  feasibility of~$\STDPM$, we know that each chunk type is
