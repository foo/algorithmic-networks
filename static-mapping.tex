...
\begin{lemma}
  Any feasible solution to $\ICTE$ places exactly $k$ nodes in the \MatchSubtree and $\deg(e) - 1$ nodes in each gadget $G(e)$ for each $e$.
...
\begin{lemma}
  In a feasible solution $\SCTE$ to $\ICTE$, each active node has exactly $1$ long assignment and $3$ short assignments, and remaining nodes have $4$ short assignments.
...
\begin{proof}
  ...
  % Assignments that are not long are short, hence the lemma holds.
