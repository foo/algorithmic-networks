...
Both the hardness of~$\FP+\RS+\MA$ and~$\FP+\RS+\NI$ variants of the $\CTE$ problem uses a reduction
...
The proof of hardness of the variant $\FP+\RS+\MA$ of the $\CTE$ problem is based on the following construction.
...
\begin{enumerate}
\item \emph{Substrate network:} We create a tree consisting of a root,
...
($\Rightarrow$) Fix a solution for~$\ITDPM$. We place a single node in every
gadget that corresponds to the chosen triples (of an arbitrary leaf of this gadget). In each gadget, we match every chunk to the node located in this gadget. This
...
In the feasible solution to $\ITDPM$ every element of the universe $X \cup Y \cup Z$ is covered, therefore every chunk is processed.

\medskip

($\Leftarrow$) Fix a solution for ~$\ICTE$ with cost at most $\Thr$.
...
In total, $x_1$ and $x_2$ incur the cost at least $14$, and thus the total chunk mapping cost $4\cdot k + 6$ exceeds the threshold $\Thr = 4\cdot k$.
...
\begin{enumerate}
  ...
  \emph{\UnqGadgets}. The {\CoverSubtree} consist of~$3\cdot k$ \emph{\ElGadgets} $G(e)$, one for each element $e\in X\cup Y\cup Z$.
  ...
  root and~$4\cdot(\deg(e)-1)+1$ leaves. There are $3\cdot k$ elements numbered from $e_1$ to $e_{3\cdot k}$.
\end{enumerate}
...
The other two sets of chunks have one replica.
...
For unique and dummy chunks we simply identify the chunk with chunk replica.
\begin{enumerate}
  \item Element chunks. For each triple $t = \langle x, y, z \rangle \in P$, we construct $3$ chunks:~$c(x, t), c(y, t),$ $c(z, t)$, i.e., $\sum_e\deg(e)$ $=3\cdot p$ chunks in total.
  Then for each element $e$ and $t\in P_e$, we a chunk $c(e, t)$ having two replicas:
  $c^{(1)}(e,t)$ and $c^{(2)}(e,t)$.
  ...
  for element $e \in X\cup Y\cup Z$ for each $t \in P_e$,
  we place replicas $c^{(2)}(t, e)$ in the leaves of the gadget $G(e)$.
  \item Unique chunks. We construct~$k$ additional chunks named
  ~$u_1, \ldots, u_k$ with one replica each, and we place those at the leaves of {\UnqGadgets} (one chunk per leaf).
  \item Dummy chunks. For each element~$e\in X\cup Y\cup Z$
  we construct additional set $\UniqueE$ of $3\cdot(\deg(e) - 1)$ chunks, with one replica each, and we place those replicas in $3\cdot (\deg(e)-1)$ out of $4\cdot (\deg(e)-1)+1)$ leaves of $G(e)$.
  Recall that remaining leaves contain replicas $c^{(2)}(t, e)$ for each $t \in P_e$. In total, we place $4\cdot (\deg(e) - 1) + 1$ replicas in $G(e)$, one replica in each leaf of the gadget.
\end{enumerate}
...
The multi-assignment factor is $m=4$, and the number of nodes to embed is~$\numNodes = k + \sum_{e}(\deg(e)-1) = k + \sum_{e}\deg(e)-3\cdot k = 3\cdot p - 2\cdot k$.
 We set the following threshold:
...
Our goal is to show that in every feasible solution, exactly $k$ \TripleGadgets{} are active (Lemma~\ref{lem:n-active-ma}), and triples that correspond to active \TripleGadgets{} in $\SVCEMB$ are the feasible solution to $\ITDPM$.

In the instance $\IVCEMB$, chunks can be matched to nodes at distance $0, 2, 4$ or $6$.
  We call the matches at distance $0$ the \emph{free matches} and the ones at distance $2$ the \emph{neighbouring matches}.
  We call the distance between the pair of leaves the \emph{short distance}, if the distance between them is at most $2$, otherwise we call it the \emph{long distance}.
...
\begin{proof}
  ...
  Each node placed in the \MatchSubtree{} has at most $3$ leaves in the short distance, hence at least one of its $\MaFactor=4$ matches is long.
  ...
  Assume the contrary, i.e., that $\Vms-k+i$ nodes were placed in the \CoverSubtree{} for $i > 0$.
  With each element gadget $G(e)$ we associate its \emph{volume} $\deg(e) - 1$. Note that the volume corresponds to the maximum number of nodes that can be placed inside the gadget without causing a long match.
  More precisely, if $\deg(e) - 1 + j_e$ nodes are placed in $G(e)$ for $j_e > 0$, the number of chunks in the gadget, $4(\deg(e)-1)+1)$ is smaller the number of chunks that these nodes should process, $4 \cdot (\deg(e)-1+j_e)$.
...
Suppose that out of $k$ nodes in the \MatchSubtree{}, $j>0$ nodes were placed in the \UnqGadgets{}.
...
Hence, exactly $k$ \TripleGadgets{} are active.
  ...
  The threshold $\Thr$ allows at most $k$ long matches and
  by Lemma~\ref{lem:no-unq-ma}, $k$ unique chunks are matched by long matches.
  Therefore, each match of $c(e, t)$ for each $e\in X\cup Y\cup Z$ and each $t \in P$ is matched by a short match.
  Hence, each active node processes the 3 chunks that are placed in its \TripleGadget.
  In each $G(e)$, one chunk $c(e, t)$ for some $t \in P$ is not matched.
