...
gadget that corresponds to the chosen triples (at an arbitrary leaf of this gadget). In each gadget, we match all three chunks to the node located in this gadget. This
...
In the feasible solution to $\ITDPM$ every element of the universe $X \cup Y \cup Z$ is covered, and therefore every chunk is processed.
...
The total chunk mapping cost $4\cdot k + 6$ exceeds the threshold $\Thr = 4\cdot k$.
...
\begin{proof}
  ...
  by Lemma~\ref{lem:no-unq-ma}, all of long matches are depleted by $k$ unique chunks.
  Therefore, each match of $c(e, t)$ for each $e\in X\cup Y\cup Z$, for each $t \in P$ is matched by a short match.
  Hence, each active node processes three chunks that are placed in its \TripleGadget.
