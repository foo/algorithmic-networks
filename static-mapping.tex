...
For a certain computational task, one spawns an adequate number of workers to process the data, whose task is to select its data portion, and process it, and aggregate the result by cooperation with other workers.
...
as the \emph{vertices}. We assume that the inter-connecting network forms an arbitrary rooted tree,
...
Since these nodes process the input data, they need to be assigned a chunk and connected to active replica.
Concretely, for each chunk~$\achunk_i$, exactly one
...
$$
\Cost(v) = \underbrace{\sum_{\achunk\in \mu(v)} \CostTrans \cdot \dist(v,\achunk)}_{\text{chunk access network}} +  \underbrace{\frac{1}{2} \cdot \sum_{v' \in \VirtualNodes\setminus\{v\}} \CostCom \cdot \dist(v,v')}_{\text{node inter-connect}},
$$
...
$\Source$ as well as to the leaves which contain replicas of their chunk.
The
...
aggregated into a single chunk vertex~$\achunk_j$  in the matching problem;
...
several variants that adimit a trivial solution. Concretely, problems with~$\FP$
...
In particular, we will show that already two replicas for each chunk are sufficient to
