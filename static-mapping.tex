...
It consists of two subtrees connected to the
...
The reduction (Theorem~\ref{th:ma-reduction}) unfolds in two parts.
...
We call a {\TripleGadget} \textit{active}, if it contains a single node at some leaf, and we call the node \textit{active} if it is placed in {\TripleGadget}.
Our goal is to show that in every feasible solution, exactly $k$ \TripleGadgets{} are active, and triples from $P$ that correspond to active \TripleGadgets{} in $\SVCEMB$ are the feasible solution to $\ITDPM$.

In the instance $\IVCEMB$, chunks can be matched to nodes at distance $0, 2, 4$ or $6$.
We call the matches at distance $0$ the \emph{free matches}, the ones at distance $2$ the \emph{neighbouring matches}, and the ones at distance $4$ or $6$ the \emph{long matches}.
\todo{Nazwij 6 match remote match}
\todo{Zmodyfikuj lemat lem:infeasible tak zeby pokazac ze sa tylko co najwyzej k matche o dlugosci 4 (a nie ze co najwyzej k matchow o dlugosci 4 lub 6)}
...
Hence, the total number of long matches would be then at least $k-j + 3\cdot j > k$, which by Lemma~\ref{lem:infeasible} would lead to infeasibility of $\SVCEMB$.
\end{proof}
...
\begin{lemma}
  In a feasible solution $\SVCEMB$ to $\IVCEMB$, all but the unique chunks are matched by a short match.
  \label{lem:short-ma}
\end{lemma}
\begin{proof}
  By Lemma~\ref{lem:infeasible}, in $\SVCEMB$ we have at most $k$ long matches.
  By Lemma~\ref{lem:no-unq-ma}, $k$ long matches are depleted by $k$ unique chunks, and remaining chunks are matched by a short match.
\end{proof}
...
\begin{proof}
  ...
  \todo{czesc 1}
  \todo{pokaz to:}
  each active node processes three chunks that are placed in its \TripleGadget and one unique chunk.
  \todo{trzeba uzyc ``at most k matches of distance 4'' i ``k active gadgets'' i ``no nodes in unique''}
  \todo{kazdy node przetwarza co najwyzej jeden z unique; jesli tak nie jest to istnieje pewien active node, ktory nie ma budzetu na long match (pidgeon), a ma tylko 3 w short distance}
  \todo{argumentuj, dlaczego jakis node nie przetwarza z innego gadzetu: niewystarczajaco long matches na unique i przetwarzanie spoza}
...
%  Hence, each active node processes three chunks that are placed in its \TripleGadget, and collectively active nodes process $3 \cdot k$ chunks.
  ...
  \todo{czesc 2 (to po prostu przepisz)}
