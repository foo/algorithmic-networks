...
For the non-augmented variant, in Section~\ref{sec:upper}, we first present a~simple
...
$\CREP$ for a setting with $(2+\eps)$-augmentation (Section~\ref{sec:crep}).
...
Section~\ref{sec:k-two}, we prove that a variant of a greedy algorithm is
7-competitive. We also demonstrate a lower bound of 3 for any deterministic
algorithm.

\item[Lower Bounds:]
By a reduction to online paging, in Section~\ref{sec:paging}, we show that
...
Section~\ref{sec:lower-bounds}, we present a stronger bound. Namely, we
